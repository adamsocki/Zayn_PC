...
void CreateRenderPass(ZaynMemory* zaynMem)
{
    ...
    // Attachment
    colorAttachment = {};
    colorAttachment.format = Zayn->vulkan.vkSwapChainImageFormat;
    ...
    if (vkCreateRenderPass(Zayn->vulkan.vkDevice, &info, nullptr, &Zayn->myIMGUI.imGuiRenderPass) !=
        VK_SUCCESS) {
...
void CreateCommandPool(ZaynMemory* zaynMem)
{
    ...
    if (vkCreateCommandPool(Zayn->vulkan.vkDevice, &poolInfo, nullptr, &zaynMem->myIMGUI.imGuiCommandPool) != VK_SUCCESS)
    {
        throw std::runtime_error("failed to create command pool!");
    }
...
VkCommandBuffer BeginSingleTimeCommands(ZaynMemory* zaynMem)
{
    ...
    Zayn->vulkan.vkSwapChainImageViews.size());
    ...
    vkAllocateCommandBuffers(Zayn->vulkan.vkDevice, &allocInfo, Zayn->myIMGUI.imGuiCommandBuffers.data());
...
void TransitionImageLayout(ZaynMemory* zaynMem, VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels)
{
    ...
    if (oldLayout == VK_IMAGE_LAYOUT_UNDEFINED && newLayout == VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL)
    {
        ...
        destinationStage = (zaynMem->vulkan.vkGraphicsQueue) ? VK_PIPELINE_STAGE_TRANSFER_BIT : VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT;
    }
...
void RenderGameObjects(ZaynMemory* zaynMem, VkCommandBuffer commandBuffer)
{
    ...
    for (int i = 0; i < zaynMem->gameObjects.size(); i++)
    {
        ...
        VkDescriptorSet& set = gameObj.material->descriptorSets[zaynMem->vulkan.vkCurrentFrame];

        vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, zaynMem->vulkan.vkGraphicsPipeline);
        vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, zaynMem->vulkan.vkPipelineLayout, 0, 1, &set, 0, nullptr);
